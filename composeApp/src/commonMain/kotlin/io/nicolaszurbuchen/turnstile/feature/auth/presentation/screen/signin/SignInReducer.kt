...
object SignInReducer : Reducer<SignInState, SignInTrigger, SignInCommand, SignEvent> {
    ...
    ): Next<SignInState, SignInCommand, SignEvent> = when (trigger) {
    ...
    private fun reduceIntent(
        state: SignInState,
        intent: SignInIntent,
    ): Next<SignInState, SignInCommand, SignEvent> = when (intent) {

        is SignInIntent.EmailChanged -> Next(
            state = state.copy(
                email = intent.value,
        ...
        is SignInIntent.PasswordChanged -> Next(
            state = state.copy(
        ...
        SignInIntent.RememberMeToggled -> Next(
            state = state.copy(rememberMe = !state.rememberMe)
        )

        SignInIntent.Submit -> if (state.canSubmit) {
            Next(
        ...
        SignInIntent.SignUpClicked -> Next(
            state = state,
            events = listOf(SignEvent.NavigateToSignUp),
        )

        SignInIntent.ForgotPasswordClicked -> Next(
            state = state,
    ...
    private fun reduceAction(
        state: SignInState,
        action: SignInAction,
    ): Next<SignInState, SignInCommand, SignEvent> = when (action) {

        is SignInAction.LoginSucceeded -> Next(
            state = state.copy(loading = false),
            events = listOf(SignEvent.NavigateHome),
        )

        is SignInAction.LoginFailedWith -> Next(
